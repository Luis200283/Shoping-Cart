import Icon from "/src/assets/icons/phone2.svg"

function HeroHeader() {
  return (
    <header className="flex items-center overflow-hidden justify-between h-100 sm:h-120 md:150 bg-linear-to-r from-slate-900 to-slate-700 ">
      <div className="rounded-3xl px-8 selection:bg-green-200 selection:text-green-900">
        <h1 className=" text-4xl sm:text-7xl md:text-8xl font-extrabold text-gray-100 text-shadow-lg/30 ">
          Shoping <br /> Online
        </h1>
        <p className="text-lg md:text-xl text-gray-400 mt-2 text-shadow-lg/30">
          palabras secundatias para el HeroHader 
        </p>
      </div>
      <div className=" flex items-center">

        <img
          src={`${Icon}`}
          alt=""
          className="size-120 lg:size-100 left-2 relative top-5"
        />
      </div>
    </header>
  );
}

export default HeroHeader;
